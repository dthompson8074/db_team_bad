...
FROM [dbo].[crime]
GROUP BY [Place], [Class Description]
...
SELECT	[Count Table].[Place]
		,[Count Table].[Class Description]
		,[Count Table].[Number of Incidents]
		,[Percentage Table].[Incident Percentage]
FROM [Count Table]
INNER JOIN [Percentage Table] ON [Count Table].[Class Description]=[Percentage Table].[Class Description] AND [Count Table].[Place]=[Percentage Table].[Place]
ORDER BY [Count Table].[Place], [Count Table].[Number of Incidents] DESC
